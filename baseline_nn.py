...
parser.add_argument('--max-attempts', type=int, default=10, help='max attempts for embedding bounds matrix')
...
for t in range(args.min_mol_id, args.max_mol_id):
    ...
    mol_init_hs = Chem.AddHs(copy.deepcopy(mol_ref))
    mol_init_hs.RemoveAllConformers()
    bm_ref_hs = molDG.GetMoleculeBoundsMatrix(mol_init_hs)
    n_est_w_hs = bm_ref_hs.shape[0]
    ...
    for repid in range(n_confs):
        ...
        mol_init_hs.RemoveAllConformers()
        bm_ref_hs_copy = copy.deepcopy(bm_ref_hs)

        # use neural net predictions to modify bounds matrix
        if args.nn_path:
            mol_nn_dist = euclidean_distances(mol_nn_pred[repid][:n_est], mol_nn_pred[repid][:n_est])

            triu_mol_nn_dist = np.triu(mol_nn_dist) + 1e-4 #+ 1e-1
            tril_mol_nn_dist = np.tril(mol_nn_dist) - 1e-4 #- 1e-1
            bm_ref_hs_copy[:n_est,:n_est] = np.triu(triu_mol_nn_dist) + np.tril(tril_mol_nn_dist)

        for atmpt in range(args.max_attempts):
            logger.info ('attempt {}'.format(atmpt))
            random_seed = np.random.randint(1, args.max_random_seed)
            #random_seed = 34
            coords = None
            try:
                coords = DG.EmbedBoundsMatrix(bm_ref_hs_copy, numZeroFail=1, maxIters=int(np.power(10, atmpt+1)), randomSeed=random_seed)
            except:
                continue
        ...
        conf = Chem.Conformer(n_est_w_hs)
        conf.SetId(0)
        for i in range(n_est_w_hs):
    ...
    if args.savepermol:
        ...
        if len(ttest_uff) > 0:
            mol_info["uff"] = ttest_mmff
            mol_info["pred_uff"] = pred_mmff
