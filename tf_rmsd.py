...
def tf_kabsch_rmsd_masked(P, Q, mask):
    ...
    P_masked = tf.matmul(mask_mat, P) + 1e-5
    Q_masked = tf.matmul(mask_mat, Q) + 1e-5
...
if __name__ == "__main__":
    ...
    P = tf.placeholder(tf.float32, [None, 3])
    Q = tf.placeholder(tf.float32, [None, 3])
    pq_rmsd = tf_kabsch_rmsd(P, Q)

    num_atoms = 6
    P_np = np.random.randn(num_atoms, 3)
    Q_np = np.random.randn(num_atoms, 3)

    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    pq_rmsd_np = sess.run(pq_rmsd, feed_dict={P:P_np, Q:Q_np})
    pq_rmsd2 = rmsd.kabsch_rmsd(P_np, Q_np)
    pdb.set_trace()
    """
    """
    P_rotated_np = sess.run(P_rotated, feed_dict={P:P_np, Q:Q_np})
    P2 = rmsd.kabsch_rotate(P_np, Q_np)
    pdb.set_trace()
    """
